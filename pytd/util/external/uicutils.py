...
__all__ = ("compileUi", "compileUiDir", "widgetPluginPath", "compileUiDirToPyDir")
...
def compileUiDirToPyDir(sUiSrcDir, sPyDestDir, recurse=False):
...
def compileUiDir(sDir, recurse=False, mapper=None, force=False, **compileUi_args):
    """compileUiDir(sDir, recurse=False, mapper=None, **compileUi_args)

    Creates Python modules from Qt Designer .ui files in a directory or
    directory tree.

    sDir is the name of the directory to scan for files whose name ends with
    ...
    False.
    mapper is an optional callable that is passed the name of the directory
    ...
    def compile_ui(ui_dir, ui_file, force=force):
        ...
        if ui_file.endswith('.ui'):
            ...
            if mapper is not None:
                py_dir, py_file = mapper(py_dir, py_file)
            ...
            if not force:
                if not newer(ui_path, py_path):
                    #print "'{0}' older than '{1}'. '{1}' left unchanged.".format(ui_file, py_file)
                    return

            print "Building '{0}' to '{1}'.".format(ui_file, py_path)
    ...
    if recurse:
        ...
        for ui in os.listdir(sDir):
            if os.path.isfile(os.path.join(sDir, ui)):
                compile_ui(sDir, ui)
